...
void routes_register(struct mg_context* context, const char* url,
    ...
    if (context && handler) {
        ...
        std::cout << Formatter_Routes::info << "Registered route: " << url << std::endl;
    } else {
        std::cerr << Formatter_Routes::err << "Failed to register route: " << url << std::endl;
    }
}
...
void routes_update(SharedData* SD) {
    ...
    Formatter_Routes::err = "[> Phoenix [Routes] > Error]: ";
        ...
        if (SD) std::cerr << ", context=" << SD->context;
        std::cerr << ")" << std::endl;
        return;
    }
    ...
    for (const auto& route : routes) {
        if (route.path && route.handler) {
            ...
            std::cerr << Formatter_Routes::err << "Invalid route configuration" << std::endl;
        }
    }
